...
What stays in `clawden-server`: Axum router, HTTP handlers (thin wrappers), WebSocket streaming, static file serving, the `clawden dashboard` subcommand.
...
- Add `clawden dashboard` subcommand (starts Axum server + opens browser)
